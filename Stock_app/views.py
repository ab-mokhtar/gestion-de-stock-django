...
def index(request):
    ...
    vente_societe=Stock.objects.values('fac__Society').annotate(item=Sum('prix'))
    ste=[]
    prices=[]
    vente_ste=dict()
    for vente in vente_societe:
       ste.append(vente['fac__Society'])
       prices.append(vente['item'])
    vente_ste['vente']=prices
    vente_ste['societe']=ste
    data_societe = dict()
    data_societe['societe']=ste
    data_societe['prix'] = prices
    data_per_soc=pd.DataFrame(data_societe)
    fig_soc = px.pie(data_per_soc, values='prix', names='societe',width=400,height=400)
    ...
    colors = ['gold', 'skyblue', '#e39ff6', 'skyblue']
    fig_soc.update_traces(textposition='inside', textinfo='percent+label',textfont_size=20,marker=dict(colors=colors, line=dict(color='white', width=2)))
    ...
    for societe in requet:
        labels.append(societe.id_sous_famille.id_famille.famille)
        famille.append(societe.id_sous_famille.designation)
        data.append(societe.prix)
   
    data_cat=dict()
  
    data_cat['categorie']=labels
    data_cat['prix']=data
    data_cat['article']=famille
    df = pd.DataFrame(data_cat)
   
    data=df.groupby(['categorie','article'],as_index=False)['prix'].sum()
    fig = px.bar(data,x='categorie', y='prix',color='article',
                 )
    fig.update_layout(
   ...
   template='simple_white',
    ...
    plot_div_cat = plot(fig, output_type='div')
    plot_div_soc = plot(fig_soc,output_type = 'div')
    lt=0
    perd=[]
    for ob in requet:
    ...
    return render(request, "./home/index.html",{"prix":price,'vente_ste':vente_ste,'plot_cat':plot_div_cat,'plot_soc':plot_div_soc,
    "trans":str(nbtrans),"labels":labels,"data":lt,"list":perd,"stck":requet,
    ...
    if request.method == 'GET':
        ...
        if formset.is_valid():
            ...
            for form in formset:
                ...
                if(famil.active == 'oui'):
                     Stock(id_sous_famille=famil,quantity=quan,reorder_level=garantie,issue_to = distination,fac=facture,ci='consommable',prix=prix).save()
                else:
                    for i in range(0,quan):
                        code=str(form.cleaned_data.get('id_sous_famille'))[0:5] + str(famil.id) + str(i)+str(i+1)+str(quan)+str(facture.id)
...
def pdf_view(request, pk):
    ...
    items = Stock.objects.all().filter(fac__id=pk)
    tot = Stock.objects.all().filter(fac__id=pk).aggregate(Sum('prix'))['prix__sum']
    context = {"ref": facture_details.ref,'total':tot,"societe": facture_details.Society,"date":facture_details.dateen,"fournisseurnom":facture_details.phone_number.nom,"fournisseurmail":facture_details.phone_number.mail,"fac_id": facture_details.id, "items":items}
...
def facture_view(request, pk):
    ...
    for item in items:
        data_facture.append(item.id)
    context = {'code_data':data_facture,"ref": facture_details.ref,"societe": facture_details.Society,"date":facture_details.dateen,"fournisseurnom":facture_details.phone_number.nom,"fournisseurmail":facture_details.phone_number.mail,"fac_id": facture_details.id, "items":items}
...
def List_tasks(request):
    ...
    context = {'tasks':tasks,'page_obj':page_obj,'name_page':'Tache'}
...
def items_Distinations(request,pk):
    items = Stock.objects.all().filter(issue_quantity__gt=0,issue_to__societe=pk)
...
def box_details(request,pk):
    ...
    reclamation=reparation_materiel.objects.values('article__ci').filter(etat='en-cours').annotate(Count('article__ci'))
